...
module.exports = db => {
  ...
  router.delete("/delete-request/:id",
    ensureLoggedIn,
    ensureHasRole(["admin", "manager"]),
    (req, res) => {
      ...
      db.collection(collection).deleteOne(
        ...
        (err, result) => {
          ...
          let query = { roles: { $in: ["hr", "vendor", "manager", "smc", "cc", "ast", "wf", "cs"] } };
          let toMailListPromise = getMailingList(query).then(mailList => {
            let scope = {
              name: request.name,
  ...
  //    - Admin all other roles should be able to view their assigned requests only
  router.get(
    "/pending",
    ensureLoggedIn,
    (req, res) => {
  ...
  router.post(
    ...
    ensureHasRole(["admin"]),
    (req, res) => {
      res.status(200).send({ validImage: true });
    }
  );
